...
function addProduct($conn)
{
    ...
    $productUPC             = $postVal("product_upc");
...
Tax_Value,Special_Value,Date_Created,SKU,Stock_Quantity,ProductMode,Age_Restriction,sale_type,status,storeadmin_id
	  ...
	  /*copy product to store admin*/
		$sql="SELECT storeadmin_id,price FROM `products_temp` where `upc`='{$productUPC}' ";
		$check= mysqli_query($conn, $sql);
		while($resultcheck =  mysqli_fetch_array($check,MYSQLI_BOTH)) {
			$storeadmin_id=$resultcheck['storeadmin_id'];
			$productSellingPrice =$resultcheck['selling_price'];
			$insertstore = "INSERT INTO product_details(product_id,description,price,sellprice,color,size,product_status,quantity,plimit,Regular_Price,Buying_Price,Tax_Status,
Tax_Value,Special_Value,Date_Created,SKU,Stock_Quantity,ProductMode,Age_Restriction,sale_type,status,storeadmin_id
			) VALUES ('{$pid}','{$productDesc}','{$productPrice}','{$productSellingPrice}','{$productColor}','{$productSize}','{$productStatus}','{$productQuantity}','{$productLimit}',		'{$productPrice}','{$productSellingPrice}','{$taxStatus}','{$taxValue}','{$Special_Value}','{$date}','{$productSKU}','{$productQuantity}','{$ProductMode}','{$ageRestriction}','{$sale_type}','{$status}','{$storeadmin_id}')";
		    $success= mysqli_query($conn, $insertstore);
		}
		
      /*copy product to store admin end*/
...
function deleteTempProduct($conn)
{
	...
	$sql="SELECT id,upc FROM `products_temp` where `id`='{$productID}'";
    $check= mysqli_query($conn, $sql);
    $resultcheck= mysqli_fetch_array($check,MYSQLI_BOTH);
    $tempId=$resultcheck['id'];
    $UPC_id=$resultcheck['upc'];
    $deleteSQL  = "DELETE FROM `products_temp` WHERE upc= '{$UPC_id}' OR id= '{$tempId}'";
